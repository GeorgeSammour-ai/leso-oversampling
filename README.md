...
LEO generates synthetic minority samples through an uncertainty-guided allocation process:

1. The feature space is partitioned into latent components using a Gaussian mixture model.
2. For each component, the class composition is evaluated and a class-mixture entropy score is computed.
3. Components with higher class overlap receive a larger share of synthetic samples.
4. Within each component, minority instances located in locally mixed neighborhoods are selected more frequently as interpolation anchors.
5. Synthetic samples are generated by interpolation between neighboring minority observations.
...
```
--output path/to/output.csv   # default: <input>_LEO.csv
